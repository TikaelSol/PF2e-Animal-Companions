    ...
    if (game.actors.filter(x => x.isOwner).filter(x => x.class?.name === "Eidolon").length) {
        ...
        if (game.user.getFlag("world", "eidolon-changes-to-be-read")) {
            new Dialog({
                ...
                content: "<p>With the release of version Companion Compendia 6.2, in order to support Summoners+ and other third party eidolons, we had to change how <b>Eidolon Abilities</b> are granted.</p><p>You need to update your Eidolon's ancestries. Simply drag and drop the same ancestry again onto your Eidolon sheet in order to update.</p>",
                buttons: {
                    one: {
